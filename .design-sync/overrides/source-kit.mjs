// forked from design-sync lib/source-kit.mjs — re-export default exports as named
// symbols in synthesis mode (React apps use `export default function Name()` which
// `export * from` silently skips, leaving those components absent from window.BibleTracker).
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const DEFAULT_RX = /export\s+default\s+(?:function|class|const|let|var)\s+([A-Z][A-Za-z0-9]*)/;
    const synthLines = [];
    for (const p of comps) {
      synthLines.push(`export * from ${JSON.stringify(p)};`);
      const m = DEFAULT_RX.exec(readText(p));
      if (m) synthLines.push(`export { default as ${m[1]} } from ${JSON.stringify(p)};`);
    }
    writeFileSync(entry, synthLines.join('\n') + '\n');
    synthEntry = true;
    console.error(
