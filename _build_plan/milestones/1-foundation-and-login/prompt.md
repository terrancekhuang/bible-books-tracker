...
- The approved visual reference is `frontend/src/prototypes/Volumes.tsx` (shelf and contents leaf) and `frontend/src/prototypes/LoginVariants.tsx` (the `board` variant, for Login). Both render at `/prototypes.html` in dev. Treat them as the design contract.
...
4. When complete, write a `milestone-log.md` in this folder (`_build_plan/milestones/1-foundation-and-login/milestone-log.md`). Structure it as follows:
   ...
   - Then include the implementation detail sections below for the next milestone's agent to reference:
     - What was built (files created, components added, routes changed, etc.)
