...
def _tz_offset() -> int:
    """Minutes east of UTC, as the client reports it (`-getTimezoneOffset()`)."""
...
def get_rhythm():
    user_id = int(get_jwt_identity())
    return jsonify(reading_history.rhythm(user_id, _tz_offset()))
...
def get_dashboard():
    user_id = int(get_jwt_identity())
    overview = reading_history.overview(user_id, _tz_offset())
